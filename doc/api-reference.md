...
- Constructors
  ...
  - `SFile::from_std_path_buf(path_buf: PathBuf) -> Result<SFile>`
  
  - `SFile::from_std_path(path: impl AsRef<Path>) -> Result<SFile>`
  
  - `SFile::from_walkdir_entry(wd_entry: walkdir::DirEntry) -> Result<SFile>`
  
  - `SFile::from_std_path_ok(path: impl AsRef<Path>) -> Option<SFile>`
  
  - `SFile::from_std_path_buf_ok(path_buf: PathBuf) -> Option<SFile>`
  
  - `SFile::from_fs_entry_ok(fs_entry: fs::DirEntry) -> Option<SFile>`
  
  - `SFile::from_walkdir_entry_ok(wd_entry: walkdir::DirEntry) -> Option<SFile>`

- Conversions (consuming / views)
  - `SFile::into_std_path_buf(self) -> PathBuf`
  
  - `SFile::std_path(&self) -> &Path`
  
  - `SFile::path(&self) -> &SPath`
...
- Getters
  ...
  - `SFile::parent_name(&self) -> &str`
  
  - `SFile::file_stem(&self) -> Option<&str>`
  
  - `SFile::stem(&self) -> &str`
  
  - `SFile::extension(&self) -> Option<&str>`
  
  - `SFile::ext(&self) -> &str`
  
  - `SFile::is_absolute(&self) -> bool`
  
  - `SFile::is_relative(&self) -> bool`

- Metadata
  - `SFile::meta(&self) -> Result<SMeta>`
  
  - `SFile::metadata(&self) -> Result<Metadata>`
  
  - `SFile::modified(&self) -> Result<SystemTime>` (deprecated)
  
  - `SFile::modified_us(&self) -> Result<i64>` (deprecated)
  
  - `SFile::file_size(&self) -> Result<u64>` (deprecated)

- Transformers
  - `SFile::canonicalize(&self) -> Result<SFile>`
  
  - `SFile::collapse(&self) -> SFile`
  
  - `SFile::into_collapsed(self) -> SFile`
  
  - `SFile::is_collapsed(&self) -> bool`

- Parent & Join
  - `SFile::parent(&self) -> Option<SPath>`
  
  - `SFile::join(&self, leaf_path: impl Into<Utf8PathBuf>) -> SPath`
  
  - `SFile::join_std_path(&self, leaf_path: impl AsRef<Path>) -> Result<SPath>`
  
  - `SFile::new_sibling(&self, leaf_path: &str) -> SPath`
  
  - `SFile::new_sibling_std_path(&self, leaf_path: impl AsRef<Path>) -> Result<SPath>`

- Diff
  - `SFile::diff(&self, base: impl AsRef<Utf8Path>) -> Option<SPath>`
  
  - `SFile::try_diff(&self, base: impl AsRef<Utf8Path>) -> Result<SPath>`

- Replace
  - `SFile::replace_prefix(&self, base: impl AsRef<str>, with: impl AsRef<str>) -> SPath`
  
  - `SFile::into_replace_prefix(self, base: impl AsRef<str>, with: impl AsRef<str>) -> SPath`

- Other
  - `SFile::strip_prefix(&self, prefix: impl AsRef<Path>) -> Result<SPath>`
  
  - `SFile::starts_with(&self, base: impl AsRef<Path>) -> bool`

- Traits (summary)
  - Display
  
  - AsRef<SFile> / AsRef<Utf8Path> / AsRef<str> / AsRef<SPath> / AsRef<Path>
  
  - From<SFile> for String / PathBuf / Utf8PathBuf / SPath
  
  - From<&SFile> for String / PathBuf / SPath
  
  - TryFrom<&str> / String / &String / PathBuf / fs::DirEntry / walkdir::DirEntry / SPath
...
- File iteration
  ...
  - `list_files(dir: impl AsRef<Path>, include_globs: Option<&[&str]>, list_options: Option<ListOptions<'_>>) -> Result<Vec<SFile>>`
  
  - Type: `GlobsFileIter` (Iterator<Item = SFile>)
